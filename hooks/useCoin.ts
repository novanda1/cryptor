...
function useCoin(args: UseCoinArgs): CategoriesResponse {
    const query = new URLSearchParams();
    Object.keys(args).filter(key => key !== 'id').map(key => query.append(key, args[key] + ''));
    const { data, error } = useSWR(`https://api.coingecko.com/api/v3/coins/${args.id}?query`, fetcher);
